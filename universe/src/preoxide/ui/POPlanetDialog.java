...
public class POPlanetDialog extends BaseDialog implements PlanetInterfaceRenderer {
  ...
  public final POPlanetRenderer planets = POUniverseMain.planetRenderer;
  ...
  public POPlanetDialog() {
    ...
    addListener(new InputListener() {
      ...
      public boolean keyDown(InputEvent event, KeyCode key) {
        if (event.targetActor == POPlanetDialog.this && (key == KeyCode.escape
            || key == KeyCode.back || key == Binding.planetMap.value.key)) {
    ...
    addCaptureListener(new ElementGestureListener() {
      ...
      public void zoom(InputEvent event, float initialDistance, float distance) {
        ...
        zoom = (Mathf.clamp(initialDistance / distance * lastZoom, state.planet.minZoom,
            getMaxZoom(state.planet)));
      }
    ...
    shown(() -> {
      if (!settings.getBool("campaignselect")
        ...
        diag.cont.row();
        diag.cont
            .label(() -> selected[0] == null ? "@campaign.none" : "@campaign." + selected[0].name)
    ...
    if (content.planets().contains(p -> p.sectors.contains(Sector::hasBase))
  ...
  public Dialog show() {
    ...
    for (SectorPreset preset : content.sectors()) {
      if (preset.unlocked() && !preset.alwaysUnlocked && !preset.sector.info.shown
  ...
  void addTech() {
    buttons.button("@techtree", Icon.tree, () -> ui.research.show()).size(200f, 54f)
        .visible(() -> mode == Mode.look).pad(2).bottom();
  }
  ...
  boolean canSelect(Sector sector) {
    ...
    if (mode == Mode.planetLaunch && sector.hasBase()) {
      return false;
    }
    ...
    if (sector.preset != null && sector.preset.requireUnlock) {
      ...
      return sector.preset.unlocked() || node == null || node.parent == null || (node.parent.content
          .unlocked()
    ...
    // use planet impl when possible
        (mode == Mode.planetLaunch ? sector.planet.generator.allowAcceleratorLanding(sector)
            : sector.planet.generator.allowLanding(sector))
        : mode == Mode.planetLaunch || sector.hasBase() || sector.near().contains(Sector::hasBase); // near
                                                                                                    // an
    // occupied
    // sector
  }
    ...
    if (launchFrom == null || (to.preset == null && !to.near().contains(launchSector))) {
      ...
      if (launchFrom == null && to.preset != null) {
        if (launchSector != null)
          return launchSector;
        launchFrom = state.planet.sectors
            .min(s -> !s.hasBase() ? Float.MAX_VALUE : s.tile.v.dst2(to.tile.v));
  ...
  public void renderSectors(Planet planet) {
    ...
    if (mode == Mode.planetLaunch && launchSector != null && selected != null && hovered == null) {
          ...
          if (selected != null && selected != sec && selected.hasBase()) {
            ...
            if (selected.info.destination == sec && selected.info.anyExports()) {
              planets.drawArc(planet, selected.tile.v, sec.tile.v,
                  Pal.place.write(Tmp.c2).a(state.uiAlpha),
                  Pal.accent.write(Tmp.c3).a(state.uiAlpha), 0.4f, 90f, 25);
            }
          }
        }
  ...
  public void renderProjections(Planet planet) {
    ...
    if (hovered != null && state.uiAlpha > 0.01f) {
      planets.drawPlane(hovered, () -> {
        Draw.color(hovered.isAttacked() ? Pal.remove : Color.white, Pal.accent,
            Mathf.absin(5f, 1f));
  ...
  void setup() {
    ...
    stack(new Element() {
      ...
      public void act(float delta) {
        if (scene.getDialog() == POPlanetDialog.this && (scene.getHoverElement() == null
            || !scene.getHoverElement().isDescendantOf(e -> e instanceof ScrollPane))) {
          scene.setScrollFocus(POPlanetDialog.this);
        }
  ...
  void rebuildExpand() {
    ...
    if (state.planet.sectors.contains(Sector::hasBase)) {
      ...
      c.button(
          bundle.get("sectorlist") + (attacked == 0 ? ""
              : "\n[red]⚠[lightgray] "
                  + bundle.format("sectorlist.attacked", "[red]" + attacked + "[]")),
          Icon.downOpen, Styles.squareTogglet, () -> sectorsShown = !sectorsShown).height(60f)
          .checked(b -> {
            Image image = (Image) b.getCells().first().get();
  ...
  void rebuildList() {
    ...
    notifs.pane(p -> {
      ...
      readd[0] = () -> {
        con.clearChildren();
        for (Sector sec : all) {
          if (sec.hasBase() && (searchText.isEmpty()
              || sec.name().toLowerCase().contains(searchText.toLowerCase()))) {
            con.button(t -> {
              t.marginRight(10f);
              t.left();
              t.defaults().growX();
              ...
              selected = sec;
              updateSelected();
            }).margin(8f).marginLeft(13f).marginBottom(6f).marginTop(6f).padBottom(3f).padTop(3f)
                .growX().checked(b -> selected == sec).row();
  ...
  public void act(float delta) {
    ...
    if (state.otherCamPos != null) {
      ...
      if (Mathf.equal(state.otherCamAlpha, 1f, 0.01f)) {
        ...
        state.camPos
            .set(Tmp.v31.set(state.otherCamPos).slerp(state.planet.position, state.otherCamAlpha)
    ...
    if (hovered != null && !mobile && state.planet.hasGrid()) {
      ...
      hoverLabel.setPosition(pos.x - Core.scene.marginLeft, pos.y - Core.scene.marginBottom,
          Align.center);
    ...
    if (state.planet.hasGrid()) {
      hovered = Core.scene.getDialog() == this ? state.planet.getSector(planets.cam.getMouseRay(),
          POPlanetRenderer.outlineRad * state.planet.radius) : null;
  ...
  void displayItems(Table c, float scl, ObjectMap<Item, ExportStat> stats, String name,
    ...
    for (var item : content.items()) {
      ...
      if (total > 1) {
        ...
        t.add(UI.formatAmount(total) + " " + Core.bundle.get("unit.perminute"))
            .color(Color.lightGray).padRight(3);
        if (++i % 3 == 0) {
          t.row();
        }
      }
  ...
  void showStats(Sector sector) {
    ...
    dialog.cont.pane(c -> {
      ...
      if (sector.info.waves && sector.hasBase()) {
        c.add(Core.bundle.get("sectors.wave") + " [accent]"
            + (sector.info.wave + sector.info.wavesPassed)).left().row();
      }

      if (sector.isAttacked() || !sector.hasBase()) {
        c.add(Core.bundle.get("sectors.threat") + " [accent]" + sector.displayThreat()).left()
            .row();
      }
      ...
      if (sector.hasBase()) {
        displayItems(c, 1f, sector.info.imports, "@sectors.import", t -> {
          sector.info.eachImport(sector.planet, other -> {
            ...
            t.add(
                Iconc.rightOpen + " " + (ic == null || ic.isEmpty() ? "" : ic + " ") + other.name())
                .padLeft(10f).row();
          });
        });
      }
    ...
    if (sector.hasBase()) {
      dialog.buttons.button("@sector.abandon", Icon.cancel,
          () -> abandonSectorConfirm(sector, dialog::hide));
    }
  ...
  void updateSelected() {
    ...
    if (sector.hasBase()) {
      stable.button("@stats", Icon.info, Styles.cleart, () -> showStats(sector)).height(40f).fillX()
          .row();
    }
    ...
    if ((sector.hasBase() && mode == Mode.look) || canSelect(sector)
      ...
      stable
          .button(
              ...
              locked ? Icon.lock : Icon.play, this::playSelected)
  ...
  void playSelected() {
    ...
    if (!planet.allowWaveSimulation && !debugSelect) {
      ...
      if (attacked != null && planet.sectors.count(s -> s.isAttacked()) < 2) {
        ...
        dialog.cont
            .add(bundle.format("sector.noswitch", attacked.name(), attacked.planet.localizedName))
    ...
    // save before launch.
    if (control.saves.getCurrent() != null && Vars.state.isGame() && mode != Mode.select) {
      try {
    ...
    if (mode == Mode.look && !sector.hasBase()) {
      shouldHide = false;
    ...
    } else if (mode == Mode.select || mode == Mode.planetLaunch) {
      listener.get(sector);
    } else {
